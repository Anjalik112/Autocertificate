...
csv_file_path = '/Users/apple/python1/csv/Group_B - Sheet4.csv'
...
def generate_certificate(name, event, event_id):
    ...
    name_y_position = 2800 
    id_x_position = 6000 
    id_y_position = 3950

    # Text color and font settings
    draw.text((name_x_position, name_y_position), name, fill=(41, 41, 41), font=font)
    draw.text((id_x_position, id_y_position), f"ID: {event_id}", fill=(41, 41, 41), font=font1)
...
def send_email(recipient_email, certificate_bytes, event, recipient_name):
    def send_email_thread():
        try:
            ...
            body = f"""Dear {name},
We are thrilled to announce that your certificate for participating in {event} Tournament is now available!
